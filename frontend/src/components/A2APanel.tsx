...
import { sendClarification } from '@/lib/api'
...
export default function A2APanel({ a2a, elapsedSeconds, onClarificationSent }: Props) {
      ...
      <div>
        <div className={ratingClass(rating)}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '.5rem' }}>
            <span className={badgeClass(rating)}>{rating}</span>
        ...
        {(a2a.valuation || a2a.valuation_method) && (
          <div style={{ marginBottom: '.6rem' }}>
            <div style={{ fontSize: '.75rem', fontWeight: 600, color: '#6b7280', marginBottom: '.3rem' }}>VALORISATION</div>
            <table className="w-full text-sm">
              <tbody>
                {a2a.valuation && <tr><td style={{ color: '#6b7280', padding: '2px 0' }}>Valorisation</td><td style={{ fontWeight: 600, textAlign: 'right' }}>{fmt(a2a.valuation)} TND</td></tr>}
                {a2a.valuation_method && <tr><td style={{ color: '#6b7280', padding: '2px 0' }}>Méthode</td><td style={{ fontWeight: 600, textAlign: 'right' }}>{a2a.valuation_method}</td></tr>}
              </tbody>
            </table>
          </div>
        )}
        ...
        {a2a.best_scenario && (
          <div style={{ marginBottom: '.6rem' }}>
            <div style={{ fontSize: '.75rem', fontWeight: 600, color: '#6b7280', marginBottom: '.3rem' }}>SCÉNARIO OPTIMAL</div>
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, padding: '.6rem' }}>
              ...
              <table className="w-full">
                <tbody>
                  {a2a.best_scenario_raise && <tr><td style={{ color: '#6b7280', fontSize: '.78rem' }}>Levée</td><td style={{ fontWeight: 600, fontSize: '.78rem', textAlign: 'right' }}>{fmt(a2a.best_scenario_raise)} TND</td></tr>}
                  {a2a.best_scenario_dilution && <tr><td style={{ color: '#6b7280', fontSize: '.78rem' }}>Dilution</td><td style={{ fontWeight: 600, fontSize: '.78rem', textAlign: 'right' }}>{a2a.best_scenario_dilution}%</td></tr>}
                  {a2a.best_scenario_post_money && <tr><td style={{ color: '#6b7280', fontSize: '.78rem' }}>Post-money</td><td style={{ fontWeight: 600, fontSize: '.78rem', textAlign: 'right' }}>{fmt(a2a.best_scenario_post_money)} TND</td></tr>}
                  {a2a.founder_after_pct && <tr><td style={{ color: '#6b7280', fontSize: '.78rem' }}>Part fondateur</td><td style={{ fontWeight: 600, fontSize: '.78rem', textAlign: 'right' }}>{a2a.founder_after_pct}%</td></tr>}
                </tbody>
              </table>
        ...
        {a2a.conflict_type && (
          <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: '.6rem', fontSize: '.8rem' }}>
            ⚠️ <strong>{a2a.conflict_type}</strong> — {a2a.conflict_message}
          </div>
        )}
      </div>
    )
