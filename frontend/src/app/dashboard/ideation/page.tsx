...
export default function IdeationPage() {
  ...
  const { setIdeationResult } = useProject();
  ...
  /** Lance l'analyse StartWise en injectant le résumé d'idéation comme contexte RAG */
  const handleLaunchStrategicAnalysis = () => {
    if (!savedSummary || !savedBusinessIdea) return;
    const shortTitleMatch = savedSummary.match(/\*\*Short Title:\*\*\s*(.+)/);
    const shortTitle = shortTitleMatch ? shortTitleMatch[1].trim() : savedBusinessIdea.slice(0, 40);
    // Inject ideation context into ProjectContext (used by marketing agents too)
    setIdeationResult({
      summary: savedSummary,
      businessIdea: savedBusinessIdea,
      sessionId: null,
      shortTitle,
    });
    localStorage.setItem('sw_ideation_summary', savedSummary);
    localStorage.setItem('sw_ideation_business_idea', savedBusinessIdea);
    // Navigate to CFO viability assessment — the next step after ideation
    router.push('/dashboard/viability-assessment');
  };
            ...
            if (runAllData) {
              ...
              if (stateData.background_image && stateData.background_image.status === 'success') {
                ...
                if (imageData.serve_url) {
                  ...
                  const serveUrl = `${backendUrl}/ideation/images/${imageData.filename}`;
        ...
        if (savedSummary && savedBusinessIdea && !backgroundImage && !imageLoading && !imageGenerationAttempted) {
          ...
          try {
            ...
            if (result.serve_url) {
              ...
              const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000';
              const serveUrl = `${backendUrl}/ideation/images/${result.filename}`;
              setBackgroundImage(serveUrl);
  ...
  const handleValidate = useCallback(async () => {
    // This will be handled by the BottomBar component
    console.log('Validation started via BottomBar');
  }, []);
    ...
    <div className="flex flex-col min-h-[70vh] w-full px-4 relative overflow-hidden">
      ...
      {hasRealData && mounted && (
        <>
          ...
          {/* ── CTA StartWise — Transition vers l'analyse stratégique ── */}
          {hasRealData && (
            <div className="w-full max-w-7xl mx-auto px-4 pb-2">
              <motion.div
                ...
                className="relative overflow-hidden rounded-2xl border border-primary/20 bg-gradient-to-r from-primary/5 via-violet-500/5 to-primary/5 p-6"
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <h3 className="text-xl font-bold text-foreground">
                      Générer l'Analyse Stratégique Complète
                    </h3>
                    <p className="text-sm text-muted-foreground max-w-md">
                      Lancez les 4 agents IA StartWise (Trend · Vision · Émotion · Créatif)
