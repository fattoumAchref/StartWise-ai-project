    ...
    if (isComplete && !showLoadingScreen && !showDashboard && sessionId) {
      ...
      setTimeout(async () => {
        try {
          ...
          // generateSummary() saved startwise_summary to localStorage.
          // Set the session gate NOW (before navigating) so ideation/page.tsx
          // passes its guard check. finalSummary is a stale closure here so we
          // cannot rely on it — read localStorage directly instead.
          sessionStorage.setItem('sw_fresh_session', '1');
          setShowLoadingScreen(false);

          // Store sibling keys needed by AppContext / ProjectContext
          const freshSummary = localStorage.getItem('startwise_summary') || '';
          const freshIdea = businessIdea || localStorage.getItem('startwise_business_idea') || '';
          if (freshSummary && freshIdea) {
            localStorage.setItem('startwise_session_id', sessionId || '');
            localStorage.setItem('startwise_business_idea', freshIdea);
            localStorage.setItem('startwise_dashboard_timestamp', new Date().toISOString());
            localStorage.setItem('sw_ideation_summary', freshSummary);
            localStorage.setItem('sw_ideation_business_idea', freshIdea);
            localStorage.setItem('sw_ideation_data', JSON.stringify({
              summary: freshSummary,
              businessIdea: freshIdea,
              sessionId,
              shortTitle: freshIdea.slice(0, 40),
            }));
          }
