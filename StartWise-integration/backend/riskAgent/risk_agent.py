...
from rag.sf_cases_rag import build_rag
...
def compute_rag_risk(description: str) -> Tuple[float, float, float]:
    """
    Queries the FAISS index (built from data/ with startup description +
    causes) and returns (risk, confidence, avg_similarity).
    """
    ...
    return risk, confidence, avg_sim
...
class RiskAgent:
    ...
    def _execute(self, description: str, db_conn) -> List[Dict]:
        ...
        llm_conflicts  = detect_conflicts_llm(agents)
        ...
        rag_risk, rag_conf, rag_similarity = compute_rag_risk(description)
    ...
    def _compute_once(self, description: str, db_conn, agents: List[AgentOutput]) -> float:
        ...
        uncertainty      = compute_uncertainty(agents)
        rag_risk, rag_conf, _ = compute_rag_risk(description)
