"""
agents/finance/a2a_models.py
============================
Google/IBM A2A Protocol data models.
...
Covers:
  ...
  - AgentCard       : agent identity, skills, capabilities (served at /.well-known/agent.json)
...
class TaskStatus:
    ...
    # Populated when state == "input-required": agent message asking the founder
...
class Task:
    ...
    def to_dict(self) -> Dict:
        # Exclude "_"-prefixed keys — those are in-memory internal refs (e.g. dataclass objects)
        # that are not JSON-serializable and are only meant for the current process.
...
class AgentCard:
    ...
    # A2A spec requires authentication so clients know the security requirements.
    # Use {"schemes": [{"type": "none"}]} for unauthenticated public agents.
