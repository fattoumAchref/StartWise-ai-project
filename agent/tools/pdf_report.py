...
def generate_pdf_report(
    ...
    if scenarios:
        scen_rows = [
            ("Pessimiste", getattr(scenarios.pessimiste, "growth_rate", None),
             getattr(scenarios.pessimiste, "arr_12m", None),
             getattr(scenarios.pessimiste, "arr_24m", None)),
            ("Realiste",   getattr(scenarios.realiste,   "growth_rate", None),
             getattr(scenarios.realiste,   "arr_12m", None),
             getattr(scenarios.realiste,   "arr_24m", None)),
            ("Optimiste",  getattr(scenarios.optimiste,  "growth_rate", None),
             getattr(scenarios.optimiste,  "arr_12m", None),
             getattr(scenarios.optimiste,  "arr_24m", None)),
        ]
        ...
        pdf.cell(40, 7, "  ARR 12 mois", fill=True)
        pdf.cell(0,  7, "  ARR 24 mois", fill=True, ln=True)
    ...
    if mc:
        _row("Runway median (P50)", f"{mc.p50} mois")
        _row("Runway pessimiste (P10)", f"{mc.p10} mois", alt=True)
        _row("Runway optimiste (P90)", f"{mc.p90} mois")
        _row("Probabilite survie 12 mois", f"{mc.proba_survie_12m:.0%}", alt=True,
             color=_GREEN if mc.proba_survie_12m > 0.7 else _RED)
        _row("Probabilite breakeven", f"{mc.proba_breakeven:.0%}",
             color=_GREEN if mc.proba_breakeven > 0.5 else _ORANGE)
        _row("Nb simulations", str(mc.n_simulations), alt=True)
        if mc.growth_mean_used:
            _row("Taux croissance utilise", f"{mc.growth_mean_used*100:.1f}%/mois")
    else:
        pdf.set_font("Helvetica", "I", 10)
