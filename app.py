...
def _process_financial_context(new_ctx: Any) -> tuple[str, list[str]]:
    """
    Fusion cumulative + pipeline de calcul + décision mode.

    Mode COLLECTE  : champs requis encore manquants → demander uniquement les manquants
    Mode ANALYSE   : tous les champs présents → afficher les KPIs, aucune question répétée
    """
    # Fusion additive — aucune donnée déjà fournie n'est perdue
    ...
    if validation is None:
        try:
            ...
            validation = ValidationResult(
                is_valid=False, data_quality_score=0.1,
                incoherences=[], missing_critical=[], questions_to_ask=[],
            )
    ...
    if not still_missing:
        # Tous les champs clés sont là → mode ANALYSE, plus de questions
        questions = []
    else:
        # Mode COLLECTE — ne demander QUE les champs encore absents
        # Filtrer missing_critical pour n'inclure que ceux réellement manquants
...
def _render_analysis_charts(ctx: Any, analysis: dict) -> None:
    ...
    with col1:
        ...
        fig_cash.add_trace(go.Scatter(
            x=months, y=pess_cash, name="Pessimiste",
            line=dict(color="#ef4444", dash="dash", width=2),
        ))
        fig_cash.add_trace(go.Scatter(
            x=months, y=real_cash, name="Réaliste",
            line=dict(color="#3b82f6", width=2.5),
        ))
        fig_cash.add_trace(go.Scatter(
            x=months, y=opti_cash, name="Optimiste",
            line=dict(color="#22c55e", dash="dot", width=2),
        ))
        fig_cash.add_hline(
            y=0, line_dash="dot", line_color="#1a1a1a", line_width=1,
        ...
        fig_cash.update_layout(
            ...
            yaxis=dict(title="Cash (DT)", gridcolor="#f0f0f0"),
            **_layout,
        )
    ...
    with col2:
        ...
        fig_rev.add_trace(go.Scatter(
            x=months, y=pess_rev, name="Pessimiste",
            line=dict(color="#ef4444", dash="dash", width=2),
        ))
        fig_rev.add_trace(go.Scatter(
            x=months, y=real_rev, name="Réaliste",
            line=dict(color="#3b82f6", width=2.5),
        ))
        fig_rev.add_trace(go.Scatter(
            x=months, y=opti_rev, name="Optimiste",
            line=dict(color="#22c55e", dash="dot", width=2),
        ))
        # Ligne breakeven = niveau de burn (quand revenus = dépenses)
        if burn > 0:
            fig_rev.add_hline(
                y=burn, line_dash="dot", line_color="#f59e0b", line_width=1.5,
                annotation_text="Breakeven", annotation_position="top right",
                annotation_font_size=10,
            )
        fig_rev.update_layout(
            title=dict(text="Revenus projetés (24 mois)", font=dict(size=13)),
            yaxis=dict(title="Revenue (DT)", gridcolor="#f0f0f0"),
            **_layout,
        )
...
def _render_financial_reply(data_text: str, questions: list[str]) -> str:
    """Render extracted data + charts + questions. Returns text for session state."""
    full_reply_parts = []
    ...
    ctx      = st.session_state.get("financial_context")
