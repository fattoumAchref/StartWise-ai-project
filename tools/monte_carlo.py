...
def run_monte_carlo(
    ...
    # ── Estimer le taux de croissance moyen ──────────────────
    if growth_mean is None:
        secteur = context.secteur.lower()
        if any(s in secteur for s in ("saas", "software", "tech")):
            growth_mean = 0.08
        elif any(s in secteur for s in ("food", "delivery", "marketplace")):
            growth_mean = 0.12
        else:
            growth_mean = 0.10
