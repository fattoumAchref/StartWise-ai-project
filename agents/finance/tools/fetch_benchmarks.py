...
from models.data_models import BenchmarkResult, FinancialContext, Phase
...
def _refresh_from_tavily(stage: str) -> bool:
    ...
    """
    try:
        from agents.finance.tools.vectorizer import BenchmarkVectorizer
...
def fetch_benchmarks(ctx: FinancialContext) -> BenchmarkResult:
    ...
    if best_similarity < SIMILARITY_THRESHOLD and stage not in _refreshed_stages:
        ...
        logger.info(f"[RAG] Stage={stage} déjà rafraîchi cette session — Tavily ignoré")
